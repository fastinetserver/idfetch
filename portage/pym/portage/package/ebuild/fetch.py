...
#	print("DIR(mysettings):",dir(mysettings))
#	print("mysettings.__doc__:",mysettings.__doc__)
#	print("DIR(mysettings.configdict):",dir(mysettings.configdict))
#	print("mysettings.__doc__:",mysettings.configdict.__doc__)
#	print("mysettings.__doc__:",mysettings.configdict.__doc__)

#	print("DIR(mysettings.configdict[pkg]): ",dir(mysettings.configdict["pkg"]))
#	print("mysettings.configdict[pkg]: ",mysettings.configdict["pkg"])

	use_twrapper_to_download = "USE_TWRAPPER" in mysettings
	if use_twrapper_to_download:
#		print("mysettings[DISTDIR]: ",mysettings["DISTDIR"])
		# add task here
		# check for lock file
		# create lock file
		...
		idfetch_pkg=dict()
		idfetch_pkg_name=mysettings.configdict["pkg"].get("PF")
		print("my: pkg_name: ",idfetch_pkg_name)
		idfetch_pkg['pkg_name']=idfetch_pkg_name
		idfetch_pkg['distfile_list']=list()
		# remove old records of the packages with the same name if they are already in the list
		for cur_pkg in idfetch_pkg_list:
			if cur_pkg['pkg_name'] == idfetch_pkg_name:
				idfetch_pkg_list.remove(cur_pkg)
		idfetch_pkg_list.append(idfetch_pkg)
	else:
		print("Tooooooooooooooooooooooooooo use twrapper: set USE_TWRAPPER=\"your_twrapper_dir\" in /etc/make.conf")

	# unlink lock file
#	my_myuris=list(myuris)
#	my_myuris2=list(myuris)
#	while my_myuris2:
#	    print ("===>>   my_myuris: ",my_myuris2.pop())
#	pkg_file = open(idfetch_settings.TASK_DIR+'/pkgs/'+idfetch_pkg_name,'w')
#	pickle.dump(my_myuris,pkg_file)
#	pkg_file.close()
#	print ("@#"*30)
	if not myuris:
		return 1
...
#		mydigests[myfile][x]
####		print ("size:",size)
####		print ("dir(orig_digest): ",dir(orig_digests))
####		print ("------------hashes:-------------------")
#		print ("hash:",orig_digests.get("x"))

#		url_file = open(idfetch_settings.TASK_DIR+'/urls/'+myfile+".urllist",'w')
#		pickle.dump(filedict[myfile][:],url_file)
#		url_file.close()
		if use_twrapper_to_download:
			idfetch_distfile=dict()
			idfetch_distfile['name']=myfile
			idfetch_distfile['url_list']=filedict[myfile][:]
			for field in mydigests[myfile]:
#				print("field:",field)
#				print("[field]:",mydigests[myfile][field])
				idfetch_distfile[field]=mydigests[myfile][field]

#		idfetch_distfiles_file = open(idfetch_settings.TASK_DIR+'/urls/dist_file_'+myfile+".urllist",'w')
#		pickle.dump(idfetch_distfile,idfetch_distfiles_file)
#		idfetch_distfiles_file.close()

			idfetch_pkg['distfile_list'].append(idfetch_distfile)
...
#			print("DIR(filedict):",dir(filedict))
#			print("DIR(filedict[myfile]):",dir(filedict[myfile]))
#			print("DIR(myfile):",dir(myfile))
#			url_file = open(idfetch_settings.TASK_DIR+'/urls/'+myfile+".urllist",'w')
#			pickle.dump(filedict[myfile][:],url_file)
#			url_file.close()
			...
			tried_locations = set()
			my_uri_list=list(uri_list)
			my_counter=0
#			while my_uri_list:
#				my_counter=my_counter+1
#				print (my_counter,") ",my_uri_list.pop())
#			print("   WILL fetch ",myfile," for you")
			if use_twrapper_to_download:
#				print("Using TWRAPPER")
#				raw_input("PAUSED: continue?")
				continue
#			raw_input("PAUSED: Press ENTER to continue?")
			while uri_list:
				...
				if fetched != 2 and has_space:
					...
					if fetched == 1:
						#resume mode:
						writemsg(_(">>> Myddd__Resuming download...\n"))
					...
					writemsg_stdout(_(">>> Myyy__Downloading '%s'\n") % \
						re.sub(r'//(.+):.+@(.+)/',r'//\1:*password*@\2/', loc))
					#print '====================loc==============='
#					writemsg_stdout("==] loc-start [== \n")
#					writemsg_stdout(loc)
#					writemsg_stdout("\n ==] loc-end [== \n")
#					writemsg_stdout("==] start-myfile [== \n")
#					writemsg_stdout(myfile)
#					writemsg_stdout("\n ==] end-myfile [== \n")
#					writemsg_stdout("==] start-mysettings [== \n")
#					writemsg_stdout(mysettings["DISTDIR"])
#					writemsg_stdout("\n ==] end-mysettings [== \n")
					#print '================ myfile ============='
					#print myfile
					#print '=========mysettings=========='
...
#			print("---------------->>>>>>>>>>>      idfetch_pkg_list_file saved       <<<<<<<<<<<<<---------------------------")
			...
			print("Error saving tasks to: ",idfetch_pkg_list_file)
